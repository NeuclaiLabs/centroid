...
def test_create_message(
    ...
    try:
        ...
        assert content["parts"] == data["parts"]
        assert content["attachments"] == data["attachments"]
...
def test_read_message(
    ...
    try:
        ...
        data = {
            "role": "user",
            "parts": "Test message for reading",
            "attachments": None,
        }
        response = client.post(
            f"{settings.API_V1_STR}/messages/?chat_id={chat_id}",
    ...
    try:
        ...
        for i in range(3):
            data = {
                ...
                "parts": f"Test message {i}",
                "attachments": {"index": i} if i % 2 == 0 else None,
            }
            client.post(
                f"{settings.API_V1_STR}/messages/?chat_id={chat_id}",
                headers=superuser_token_headers,
...
def test_normal_user_cannot_delete_message(
    ...
    try:
        ...
        data = {
            "role": "user",
            "parts": "Message that normal user shouldn't delete",
            "attachments": None,
        }
        response = client.post(
            f"{settings.API_V1_STR}/messages/?chat_id={chat_id}",

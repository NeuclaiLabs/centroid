...
export const documenterDocumentHandler = createDocumentHandler<'documenter'>({
  kind: 'documenter',
  onCreateDocument: async ({ title, dataStream, session }) => {
    try {
      ...
      dataStream.writeData({
        ...
        taskId,
        status: 'in_progress',
        message: 'Starting documentation task...',
      });
      ...
      while (attempts < maxAttempts) {
        ...
        dataStream.writeData({
          ...
          message: `Documentation task ${statusData.status}...`,
        });
        ...
        if (statusData.status === 'completed' || statusData.status === 'error') {
          ...
          if (resultResponse.ok) {
            ...
            if (statusData.status === 'completed') {
              ...
              throw new Error(taskData.error || 'SDLC documentation task failed');
            }
          }
          break;
        }
      ...
      if (attempts >= maxAttempts) {
        throw new Error('Documentation task timeout - taking longer than expected');
      }
    ...
    try {
      ...
      while (attempts < maxAttempts) {
        ...
        dataStream.writeData({
          ...
          taskId,
          status: statusData.status,
          message: `Documentation update ${statusData.status}...`,
        });
        ...
        if (statusData.status === 'completed' || statusData.status === 'error') {
          ...
          if (resultResponse.ok) {
            ...
            if (statusData.status === 'completed') {
              ...
              throw new Error(taskData.error || 'SDLC documentation update task failed');
            }
          }
          break;
        }
      ...
      if (attempts >= maxAttempts) {
        throw new Error('Documentation update task timeout - taking longer than expected');
      }

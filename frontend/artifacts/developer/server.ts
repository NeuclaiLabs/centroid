...
export const developerDocumentHandler = createDocumentHandler<'developer'>({
  kind: 'developer',
  onCreateDocument: async ({ title, dataStream, session }) => {
    try {
        ...
        if (statusData.status === 'COMPLETED' || statusData.status === 'ERROR') {
          ...
          if (resultResponse.ok) {
            ...
            if (statusData.status === 'COMPLETED') {
              ...
              throw new Error(taskData.error || 'SDLC task failed');
            }
          }
          break;
        }
      ...
      if (attempts >= maxAttempts) {
        throw new Error('Task timeout - taking longer than expected');
      }
  ...
  onUpdateDocument: async ({ document, description, dataStream, session }) => {
    try {
      ...
      dataStream.writeData({
        ...
        taskId,
        status: 'in_progress',
        message: 'Updating development task...',
      });
      ...
      while (attempts < maxAttempts) {
        ...
        dataStream.writeData({
          ...
          taskId,
          status: statusData.status,
          message: `Update ${statusData.status}...`,
        });
        ...
        if (statusData.status === 'COMPLETED' || statusData.status === 'ERROR') {
          ...
          if (resultResponse.ok) {
            ...
            if (statusData.status === 'COMPLETED') {
              ...
              throw new Error(taskData.error || 'SDLC update task failed');
            }
          }
          break;
        }
      ...
      if (attempts >= maxAttempts) {
        throw new Error('Update task timeout - taking longer than expected');
      }

...
export const reviewerDocumentHandler = createDocumentHandler<'reviewer'>({
  kind: 'reviewer',
  onCreateDocument: async ({ title, dataStream, session }) => {
    try {
      ...
      dataStream.writeData({
        ...
        taskId,
        status: 'in_progress',
        message: 'Starting code review task...',
      });
      ...
      while (attempts < maxAttempts) {
        ...
        dataStream.writeData({
          ...
          message: `Review task ${statusData.status}...`,
        });
        ...
        if (statusData.status === 'completed' || statusData.status === 'error') {
          ...
          if (resultResponse.ok) {
            ...
            if (statusData.status === 'completed') {
              ...
              throw new Error(taskData.error || 'SDLC review task failed');
            }
          }
          break;
        }
      ...
      if (attempts >= maxAttempts) {
        throw new Error('Review task timeout - taking longer than expected');
      }
    ...
    try {
      ...
      while (attempts < maxAttempts) {
        ...
        dataStream.writeData({
          ...
          taskId,
          status: statusData.status,
          message: `Review update ${statusData.status}...`,
        });
        ...
        if (statusData.status === 'completed' || statusData.status === 'error') {
          ...
          if (resultResponse.ok) {
            ...
            if (statusData.status === 'completed') {
              ...
              throw new Error(taskData.error || 'SDLC review update task failed');
            }
          }
          break;
        }
      ...
      if (attempts >= maxAttempts) {
        throw new Error('Review update task timeout - taking longer than expected');
      }

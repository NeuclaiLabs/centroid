...
interface ArchitectArtifactMetadata {
  result: ArchitectResult | null;
}

export const architectArtifact = new Artifact<'architect', ArchitectArtifactMetadata>({
  kind: 'architect',
  description: 'Display system architecture designs with components, patterns, and technical specifications.',
  ...
  content: ({
      ...
      return (
        <div className="flex flex-col h-full bg-black font-mono">
          <div className="flex items-center gap-2 px-4 py-3 border-b border-zinc-800 bg-zinc-900/50">
            <div className="size-4 text-purple-400">
              <Terminal size={16} />
            </div>
            <span className="text-zinc-300 text-sm">claude-architect</span>
    ...
    const task = typeof taskText === 'string' ? taskText :
      Array.isArray(taskText) ? taskText.find(c => c.type === 'text')?.text || 'Architecture Design' :
      'Architecture Design';
                ...
                <div key={message.id || `message-${index}`} className="space-y-1">
                  ...
                  <div className="flex items-start gap-2">
                    {isUser ? (
                      ...
                      <div className="size-4 text-purple-400 shrink-0 mt-0.5">
                        <BotIcon />
                      </div>
                    )}
  ...
  actions: [
    {
      ...
      description: 'Copy architecture to clipboard',
      onClick: ({ metadata }) => {
        if (metadata?.result) {
    ...
    },
    {
      ...
      description: 'Download architecture as text file',
      onClick: ({ metadata }) => {
        if (metadata?.result) {
          const assistantMessages = metadata.result.filter(msg => msg.role === 'assistant');
          const finalMessage = assistantMessages[assistantMessages.length - 1];
          ...
          if (finalContent) {
            ...
            a.href = url;
            a.download = `${task.replace(/[^a-zA-Z0-9]/g, '_')}_architecture.txt`;
            document.body.appendChild(a);
            a.click();
            document.body.removeChild(a);
            URL.revokeObjectURL(url);
            toast.success('Architecture file downloaded!');
          }
        }
      },
      isDisabled: ({ metadata }) => {
  ...
  toolbar: [
    {
      icon: <RefreshIcon />,
      description: 'Regenerate architecture',
      onClick: ({ appendMessage }) => {
        appendMessage({
          role: 'user',
          content: 'Please regenerate the architecture with additional patterns or improvements.',
        });
      },
    },
    {
      ...
      onClick: ({ appendMessage }) => {
        appendMessage({
          role: 'user',
          content: 'Please create detailed technical specifications based on this architecture.',
        });
      },
    },
  ],

...
interface PlannerArtifactMetadata {
  result: PlannerResult | null;
}

export const plannerArtifact = new Artifact<'planner', PlannerArtifactMetadata>({
  kind: 'planner',
  description: 'Display project planning results with comprehensive plans, phases, and recommendations.',
  ...
  content: ({
      ...
      return (
        <div className="flex flex-col h-full bg-black font-mono">
          <div className="flex items-center gap-2 px-4 py-3 border-b border-zinc-800 bg-zinc-900/50">
            ...
            <span className="text-zinc-300 text-sm">claude-planner</span>
    ...
    const task = typeof taskText === 'string' ? taskText :
      Array.isArray(taskText) ? taskText.find(c => c.type === 'text')?.text || 'Project Planning' :
      'Project Planning';
                ...
                <div key={message.id || `message-${index}`} className="space-y-1">
                  ...
                  <div className="flex items-start gap-2">
                    {isUser ? (
                      ...
                      <div className="size-4 text-blue-400 shrink-0 mt-0.5">
                        <BotIcon />
                      </div>
                    )}
  ...
  actions: [
    ...
    },
    {
      ...
      description: 'Download plan as text file',
      onClick: ({ metadata }) => {
        if (metadata?.result) {
          const assistantMessages = metadata.result.filter(msg => msg.role === 'assistant');
          const finalMessage = assistantMessages[assistantMessages.length - 1];
          ...
          if (finalContent) {
            ...
            a.href = url;
            a.download = `${task.replace(/[^a-zA-Z0-9]/g, '_')}_plan.txt`;
  ...
  toolbar: [
    {
      ...
      onClick: ({ appendMessage }) => {
        appendMessage({
          role: 'user',
          content: 'Please regenerate the plan with additional detail or improvements.',
        });
      },
    },
    {
      ...
      onClick: ({ appendMessage }) => {
        appendMessage({
          role: 'user',
          content: 'Please create a detailed implementation guide based on this plan.',
        });
      },
    },
  ],

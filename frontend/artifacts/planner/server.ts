...
export const plannerDocumentHandler = createDocumentHandler<'planner'>({
  kind: 'planner',
  onCreateDocument: async ({ title, dataStream, session }) => {
    try {
      ...
      dataStream.writeData({
        ...
        taskId,
        status: 'in_progress',
        message: 'Starting planning task...',
      });
      ...
      while (attempts < maxAttempts) {
        ...
        dataStream.writeData({
          ...
          message: `Planning task ${statusData.status}...`,
        });
        ...
        if (statusData.status === 'completed' || statusData.status === 'error') {
          ...
          if (resultResponse.ok) {
            ...
            if (statusData.status === 'completed') {
              ...
              throw new Error(taskData.error || 'SDLC planning task failed');
            }
          }
          break;
        }
      ...
      if (attempts >= maxAttempts) {
        throw new Error('Planning task timeout - taking longer than expected');
      }
  ...
  onUpdateDocument: async ({ document, description, dataStream, session }) => {
    try {
      ...
      dataStream.writeData({
        ...
        taskId,
        status: 'in_progress',
        message: 'Updating planning task...',
      });
      ...
      while (attempts < maxAttempts) {
        ...
        dataStream.writeData({
          ...
          taskId,
          status: statusData.status,
          message: `Planning update ${statusData.status}...`,
        });
        ...
        if (statusData.status === 'completed' || statusData.status === 'error') {
          ...
          if (resultResponse.ok) {
            ...
            if (statusData.status === 'completed') {
              ...
              throw new Error(taskData.error || 'SDLC planning update task failed');
            }
          }
          break;
        }
      ...
      if (attempts >= maxAttempts) {
        throw new Error('Planning update task timeout - taking longer than expected');
      }

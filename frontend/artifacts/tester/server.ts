...
export const testerDocumentHandler = createDocumentHandler<'tester'>({
  kind: 'tester',
    ...
    try {
      ...
      while (attempts < maxAttempts) {
        ...
        dataStream.writeData({
          ...
          message: `Testing task ${statusData.status}...`,
        });
        ...
        if (statusData.status === 'completed' || statusData.status === 'error') {
          ...
          if (resultResponse.ok) {
            ...
            if (statusData.status === 'completed') {
              ...
              throw new Error(taskData.error || 'SDLC testing task failed');
            }
          }
          break;
        }
      ...
      if (attempts >= maxAttempts) {
        throw new Error('Testing task timeout - taking longer than expected');
      }
    ...
    try {
      ...
      while (attempts < maxAttempts) {
        ...
        dataStream.writeData({
          ...
          taskId,
          status: statusData.status,
          message: `Testing update ${statusData.status}...`,
        });
        ...
        if (statusData.status === 'completed' || statusData.status === 'error') {
          ...
          if (resultResponse.ok) {
            ...
            if (statusData.status === 'completed') {
              ...
              throw new Error(taskData.error || 'SDLC testing update task failed');
            }
          }
          break;
        }
      ...
      if (attempts >= maxAttempts) {
        throw new Error('Testing update task timeout - taking longer than expected');
      }

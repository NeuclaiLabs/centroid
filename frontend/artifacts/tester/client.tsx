...
interface TesterArtifactMetadata {
  result: TesterResult | null;
}

export const testerArtifact = new Artifact<'tester', TesterArtifactMetadata>({
  kind: 'tester',
  description: 'Display comprehensive test plans with strategies, test cases, and quality assurance recommendations.',
  ...
  content: ({
      ...
      return (
        <div className="flex flex-col h-full bg-black font-mono">
          <div className="flex items-center gap-2 px-4 py-3 border-b border-zinc-800 bg-zinc-900/50">
            <div className="size-4 text-orange-400">
              <Terminal size={16} />
            </div>
            <span className="text-zinc-300 text-sm">claude-tester</span>
    ...
    const task = typeof taskText === 'string' ? taskText :
      Array.isArray(taskText) ? taskText.find(c => c.type === 'text')?.text || 'Test Planning' :
      'Test Planning';
                ...
                <div key={message.id || `message-${index}`} className="space-y-1">
                  ...
                  <div className="flex items-start gap-2">
                    {isUser ? (
                      ...
                      <div className="size-4 text-orange-400 shrink-0 mt-0.5">
                        <BotIcon />
                      </div>
                    )}
  ...
  actions: [
    ...
    },
    {
      ...
      description: 'Download test plan as text file',
      onClick: ({ metadata }) => {
        if (metadata?.result) {
          const assistantMessages = metadata.result.filter(msg => msg.role === 'assistant');
          const finalMessage = assistantMessages[assistantMessages.length - 1];
          ...
          if (finalContent) {
            ...
            a.href = url;
            a.download = `${task.replace(/[^a-zA-Z0-9]/g, '_')}_testplan.txt`;
            document.body.appendChild(a);
            a.click();
            document.body.removeChild(a);
            URL.revokeObjectURL(url);
            toast.success('Test plan file downloaded!');
          }
        }
      },
      isDisabled: ({ metadata }) => {
  ...
  toolbar: [
    {
      icon: <RefreshIcon />,
      description: 'Regenerate test plan',
      onClick: ({ appendMessage }) => {
        appendMessage({
          role: 'user',
          content: 'Please regenerate the test plan with additional test cases or improvements.',
        });
      },
    },
    {
      icon: <FileTextIcon />,
      description: 'Create test automation guide',
      onClick: ({ appendMessage }) => {
        appendMessage({
          role: 'user',
          content: 'Please create a test automation implementation guide based on this test plan.',
        });
      },
    },
  ],

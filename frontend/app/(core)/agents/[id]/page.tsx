...
import { SchemaDialog } from "@/app/(core)/components/schema-dialog";
...
import { MCPServerHeader } from "@/app/(core)/components/mcp-server-header";
...
export default function MCPServerPage({ params }: PageProps) {
	...
	const { changeServerState, updateEnvironmentVariable, toggleToolStatus } =
		useMCPServers({ isAgent: true });

	const {
		data: server,
		isLoading,
		mutate,
	} = useSWR<MCPServer>(`/api/mcp/servers/${id}?is_agent=true`, async (url: string) => {
